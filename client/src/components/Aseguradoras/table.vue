...
<div>
	<!--Edit dialog form -->
	<el-dialog width="40%" top="5vh" :title="aseguradora.nombre" :visible.sync="editFormVisible">
		<AseguradorasEditForm></AseguradorasEditForm>
		<span slot="footer" class="dialog-footer">
			<el-button size="mini" @click="editFormVisible = false">Cancelar</el-button>
			<el-button size="mini" type="primary" @click="editAseguradora">Actualizar</el-button>
		</span>
	</el-dialog>
	<!--Create dialog form -->
	<el-dialog width="40%" top="5vh" title="Nueva aseguradora" :visible.sync="createFormVisible">
		<AseguradorasCreateForm></AseguradorasCreateForm>
		<span slot="footer" class="dialog-footer">
			<el-button size="mini" @click="createFormVisible = false">Cancelar</el-button>
			<el-button size="mini" type="primary" @click="createAseguradora">Crear</el-button>
		</span>
	</el-dialog>
	<!-- searchbar -->
	...
	<el-table
    v-loading.body="loadingAseguradorasTable"
	size="mini"
	ref="aseguradorasTable"
	stripe
	max-height="250"
...
//componentes
import AseguradorasCreateForm from '@/components/Aseguradoras/createForm'
import AseguradorasEditForm from '@/components/Aseguradoras/editForm'

export default {
	name: 'AseguradorasTable',
	data () {
      	return {
	...
	computed: {
        ...
        ...mapState('aseguradoras', [
			...
			'aseguradorasList',
            'headings',
            'loadingAseguradorasTable',
		]),
        filtered(){
			
				if(this.filter !== ''){
					let type = this.selectTypeOfSearch.toLowerCase()
					return this.aseguradorasList.filter(servicio => {
				...
				return this.aseguradorasList
		},
	},
	components: {
        AseguradorasCreateForm,
        AseguradorasEditForm,
	},
    methods: {
        reloadTable(){
        ...
        exportTable(){
			exportService.toXLS(this.aseguradorasList, 'Aseguradoras', true)
		},
		handleAction(e, row){
            if(e == 'create'){
                this.aseguradoraReset()
        ...
        handleCurrentTableChange(val) {
			if(val == null){
				this.$refs.aseguradorasTable.setCurrentRow(val);
				return
			}
			this.setFullAseguradora(val)
			this.$refs.aseguradorasTable.setCurrentRow(val);
		},
		back(){
			
		},
		...mapMutations('aseguradoras', [
            'setFullAseguradora',
            'aseguradoraReset',
		]),
